function [ax, log, Sa] = v3Calc2(car, model, v, radius_d, itt_lim, err_margin)
%   Author:     Jakub Horsky
...
%     squeeze(Fz)
    ...
    if (ax_returned == ax_breaks_limit) 
        limiting_factor = 3; %if limited by break bias/tire grip
    else
...
%Implementing a limit to ax based on the brakes.

ax_Braking_Lockup_Limit = -2*(car.LF_front*car.brakeSystem.brake_bias + car.LF_rear*(1-car.brakeSystem.brake_bias))/car.mass;

if abs(ax) >= abs(ax_Braking_Lockup_Limit) && ax < 0
    ax = ax_Braking_Lockup_Limit;
    %warning('Braking lockup limit.')


%Updating the parameters required with the final acceleration (limited to
%the braking lock-up limit if required). Note: the base for this code has
%come from the above loop, created by the IFR simulations team prior.
...
[sl,sa] = findslsa(Fx_bb(1,:,:),Fy(1,:,:),Fz(1,:,:),model,car);
