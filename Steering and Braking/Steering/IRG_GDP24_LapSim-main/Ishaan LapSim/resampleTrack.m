function [x_s,y_s,distance,curvature_filtered,radius_d,delta_heading,delta_heading_max] = resampleTrack(x,y,n)
    ...
    %% Calculate the heading of each turn from the perspective of the driver (assuming the driver is perfect in this case on following the line of the track).
    
    for i = 1:n-1
        delta_heading(i) = ((theta_filtered(i+1) - theta_filtered(i)) * (180/pi))/(distance(i+1) - distance(i)); %Calculates the heading change at each point, used for steering variation across the lap.
    end
    
    delta_heading(end+1) = 0;
    
    delta_heading_max = max(delta_heading);
