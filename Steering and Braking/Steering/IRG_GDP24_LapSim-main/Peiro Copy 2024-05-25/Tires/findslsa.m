function [SL_max,Sa_max] = findslsa(Fx,Fy,F_z, model, car)
    ...
    for i = 1:2
        ...
        for j = 1:2
            ...
            IA = car.IA;
            P = car.tyre_pressure; % hardcode for now
            ...
            for k = 1:n
                ...
                for c = 1:n
                    ...
                    [Fxsys,Fysys] = combinedPacejkaFix(SA(c), SL(k), lat_coeff, long_coeff, FZ);

                    if abs(Fxsys * Scaling_factor - Fx(1,i,j)) <= err1 && abs(Fysys * Scaling_factor - Fy(1,i,j)) <= err2
    
                        SL_max(i,j) =  SL(k);
    
                        err1 = abs(Fxsys * Scaling_factor - Fx(1,i,j));

                        Sa_max(i,j) =  SA(c);
    
                        err2 = abs(Fysys * Scaling_factor - Fy(1,i,j));
